...
public class ATM {
    ...
    Map<NominalValues, Integer> allMoney = new HashMap<NominalValues, Integer>();
    ...
    public void printRemainingBanknotes() {
        for (Map.Entry<NominalValues, Integer> entry : allMoney.entrySet()) {
            System.out.println("Колличество банкнот по " + entry.getKey().getNominal() + " рублей " + entry.getValue() + " шт");
        }
        System.out.println("----------------------------------");
    }
